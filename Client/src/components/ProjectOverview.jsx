...
const ProjectOverview = () => {
    ...
    // Format status string: "ON_HOLD" → "On Hold"
                ...
                {projects.length === 0 ? (
                    ...
                    <div className="divide-y divide-zinc-200 dark:divide-zinc-800">
                        {projects.slice(0, 5).map((project) => {
                            ...
                            // ✅ Guard against undefined status/priority
                                ...
                                >
                                    <div className="flex items-start justify-between mb-3">
                                        ...
                                        <div className="flex items-center gap-2 ml-4">
                                            {/* ✅ Only render status badge if status exists */}
                                            {status && (
                                                <span className={`text-xs px-2 py-1 rounded ${statusColors[status] || 'bg-zinc-200 text-zinc-800'}`}>
                                                    {formatStatus(status)}
                                                </span>
                                            )}
                                            {/* ✅ Only render priority dot if priority exists */}
                                            {priority && (
                                                <div className={`w-2 h-2 rounded-full border-2 ${priorityColors[priority] || 'border-zinc-300'}`} />
                                            )}
                                        </div>
                                    </div>
